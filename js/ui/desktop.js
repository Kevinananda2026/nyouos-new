...
const Desktop = {
    ...
    apps: [
        ...
        { id: 'scratch', name: 'Scratch', icon: 'Theme/Icon/App_icon/snake.png', isPWA: true },
    ...
    initEnhancedSystem() {
        ...
        this._wireEnhancedIntegrations();

        console.log('[NyouOS] Enhanced system modules ready:', modulesLoaded.join(', '));
    },

    _wireEnhancedIntegrations() {
        var self = this;

        if (window.OSContextMenu && this.iconsContainer) {
            this.iconsContainer.addEventListener('contextmenu', function(e) {
                ...
                if (!icon.classList.contains('selected')) {
                    ...
                    self.getSelectedIconElements();
                    self.selectedIcon = icon;
                }

                var nodeId = icon.dataset.nodeId;
                var node = State.findNode(nodeId);
                if (node) {
                    OSContextMenu.showMenu(e.clientX, e.clientY, self._getFileMenuItems(node), { type: 'file', node: node });
                }
            });
        }

        if (window.OSContextMenu && document.getElementById('taskbar-apps')) {
            var taskbarApps = document.getElementById('taskbar-apps');
            taskbarApps.addEventListener('contextmenu', function(e) {
                var btn = e.target.closest('.taskbar-app');
                if (!btn) return;
                e.preventDefault();
                e.stopPropagation();
                OSContextMenu.showTaskbarMenu(e.clientX, e.clientY);
            });
        }
        ...
        if (window.OSShortcuts) {
            ...
            OSShortcuts.register('win+l', function() {
                if (typeof State !== 'undefined' && State.lock) {
                    State.lock();
                }
            });
            ...
            OSShortcuts.register('win+r', function() {
                if (typeof OSShortcuts !== 'undefined') {
                    OSShortcuts._showRunDialog && OSShortcuts._showRunDialog();
                }
            });

            OSShortcuts.register('f5', function() {
                self.renderIcons();
            });
        }

        if (window.OSActionCenter && typeof State !== 'undefined') {
            State.on('notificationAdd', function(notif) {
                if (OSActionCenter.addNotification && notif) {
                    OSActionCenter.addNotification(notif.title || '通知', notif.message || '', notif.icon || '📢');
                }
            });
        }

        console.log('[NyouOS] Enhanced integrations wired');
    },

    _getFileMenuItems(node) {
        var items = [];
        ...
        items.push({
            ...
            onClick: function() {
                if (typeof FilesApp !== 'undefined' && typeof FilesApp.openNodeWithDefaultApp === 'function') {
                    FilesApp.openNodeWithDefaultApp(node);
                }
            }
        });
        ...
        items.push({
            ...
            shortcut: 'Delete',
            onClick: function() {
                if (typeof FilesApp !== 'undefined' && typeof FilesApp.deleteNode === 'function') {
                    FilesApp.deleteNode(node);
                }
            }
        });

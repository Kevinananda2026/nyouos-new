...
function initModules() {
    ...
    Surf.init();
    if (typeof Screenshot !== 'undefined' && typeof Screenshot.init === 'function') {
        Screenshot.init();
    }
    initGlobalShortcuts();
...
(function() {
    const apps = [
        'bilibili', 'douyin', 'kuaishou', 'youku', 'tencent-video',
        'iqiyi', 'bilibili-live', 'netease-music', 'qq-music', 'kugou',
        'xiami-music', 'kuwo-music', 'taobao', 'jd', 'xianyu', 'pdd',
        'meituan-app', 'eleme', 'taobaoshangou', 'dangdang', 'weibo',
        'zhihu', 'douban', 'xiaohongshu', 'chatgpt', 'deepseek', 'qwen',
        'kimi', 'doubao', 'baidu', 'baidu-map', 'amap', 'youdao',
        'baidu-translate', 'baidu-netdisk', 'wechat', 'dingding', 'feishu',
        'teams', 'qq', 'wangyiyan', 'office', 'canva', 'shimo', 'ticktick',
        'notional', 'evernote', 'wps', 'alipay', 'didi', 'meituan-waimai',
        'eleme-waimai', 'trip', 'fliggy', 'dianping', 'chinadaily', 'ithome',
        'thepaper', '36kr', 'snake', 'puke', 'github', 'stackoverflow',
        'juejin', 'csdn', 'coursera', 'zhihu-edu', 'weread', 'zhongtai',
        'scratch'
    ];
    const loadApp = (id) => {
        return new Promise((resolve) => {
            if (typeof PWALoader !== 'undefined' && PWALoader.isRegistered && PWALoader.isRegistered(id)) {
                resolve();
                return;
            }
            const script = document.createElement('script');
            script.src = `js/third_parts_apps/${id}.js`;
    ...
    async function loadAll() {
        ...
        console.log('[PWA] All third-party apps loaded');
    }
...
(function() {
    if (typeof BlueScreen !== 'undefined') return;
    var script = document.createElement('script');
    script.src = 'js/ui/bsod.js';
    document.head.appendChild(script);
})();
...
function initGlobalShortcuts() {
    ...
    document.addEventListener('keydown', (e) => {
        ...
        switch (key) {
            case 'f':
                if (typeof Surf !== 'undefined' && typeof Surf.toggle === 'function') {
                    Surf.toggle();
                    handled = true;
                }
                break;
    ...
    document.addEventListener('keydown', (e) => {
        if (e.ctrlKey && e.shiftKey && e.key.toLowerCase() === 's') {
            e.preventDefault();
            if (typeof Screenshot !== 'undefined' && typeof Screenshot.take === 'function') {
                Screenshot.take();
            }
        }
    });
...
// 欢迎消息
console.log('%c欢迎使用 NyouOS！', 'color: #0078d4; font-size: 14px; font-weight: bold;');

...
const FilesApp = {
    ...
    SYSTEM_ROOT_FOLDERS: ['desktop', 'documents', 'pictures', 'downloads', 'recycle', 'system'],
    ...
    renderContentPage(pageEl) {
        ...
        touchBar.className = 'files-touch-action-bar' + (this.isTouchDevice ? ' visible' : '');
        touchBar.id = `${this.windowId}-files-touch-bar`;
        touchBar.innerHTML = `
            <div class="touch-bar-inner" id="${this.windowId}-files-touch-inner">
                <button class="touch-bar-btn" data-action="open" title="${t('files.open')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Folder Open.svg" alt="">
                    <span>${t('files.open')}</span>
                </button>
                <button class="touch-bar-btn" data-action="copy" title="${t('files.copy')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Copy.svg" alt="">
                    <span>${t('files.copy')}</span>
                </button>
                <button class="touch-bar-btn" data-action="cut" title="${t('files.cut')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Scissors.svg" alt="">
                    <span>${t('files.cut')}</span>
                </button>
                <button class="touch-bar-btn" data-action="paste" title="${t('files.paste')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Clipboard.svg" alt="">
                    <span>${t('files.paste')}</span>
                </button>
                <button class="touch-bar-btn" data-action="rename" title="${t('files.rename')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Edit.svg" alt="">
                    <span>${t('files.rename')}</span>
                </button>
                <button class="touch-bar-btn" data-action="delete" title="${t('files.delete')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Trash.svg" alt="">
                    <span>${t('files.delete')}</span>
                </button>
                <button class="touch-bar-btn" data-action="properties" title="${t('files.properties')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Information Circle.svg" alt="">
                    <span>${t('files.properties')}</span>
                </button>
                <button class="touch-bar-btn" data-action="select-all" title="${t('files.select-all')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Select Box.svg" alt="">
                    <span>${t('files.select-all')}</span>
                </button>
            </div>
        `;
        main.appendChild(touchBar);
    ...
    addStyles() {
        ...
        style.textContent = `
            .files-app { display: flex; flex-direction: column; height: 100%; position: relative; }
            ...
            .files-main { flex: 1; display: flex; position: relative; overflow: hidden; }
            ...
            .files-touch-action-bar.visible {
                transform: translateY(0);
            }
            .blur-disabled .files-touch-action-bar {
                backdrop-filter: none;
                -webkit-backdrop-filter: none;
                background: var(--bg-primary);
            }
            .files-touch-action-bar .touch-bar-inner {
                display: flex;
                align-items: center;
            ...
            .files-touch-action-bar .touch-bar-inner::-webkit-scrollbar {
                display: none;
            }
            .files-touch-action-bar .touch-bar-btn {
                flex: 1 0 auto;
                min-width: 56px;
                display: flex;
                ...
                font-size: 11px;
                cursor: pointer;
                transition: background var(--transition-fast, 0.15s), opacity var(--transition-fast, 0.15s);
                white-space: nowrap;
                -webkit-tap-highlight-color: transparent;
            }
            .files-touch-action-bar .touch-bar-btn:hover,
            .files-touch-action-bar .touch-bar-btn:active {
                background: rgba(var(--accent-rgb, 0, 120, 212), 0.15);
            }
            .files-touch-action-bar .touch-bar-btn:disabled {
                opacity: 0.35;
                pointer-events: none;
            }
            .files-touch-action-bar .touch-bar-btn.danger {
                color: #c42b1c;
            }
            .files-touch-action-bar .touch-bar-btn img {
                width: 20px;
                height: 20px;
            }
            .dark-mode .files-touch-action-bar .touch-bar-btn:hover,
            .dark-mode .files-touch-action-bar .touch-bar-btn:active {
                background: rgba(var(--accent-rgb, 100, 180, 255), 0.2);
            }
            body.fluent-v2 .files-touch-action-bar {
            ...
            body.fluent-v2.dark-mode .files-touch-action-bar {
                box-shadow: 0 -4px 24px rgba(0, 0, 0, 0.4);
            }
            .files-main {
                position: relative;
            }
            .files-content.touch-bar-open {
                padding-bottom: 90px;
            }
        `;
    ...
    bindEvents() {
        ...
        filesList.addEventListener('click', (e) => {
            ...
            if (item) {
                if (this.isTouchDevice) {
                    // 触屏：已选中则打开，否则选中
                    if (this.selectedItems.length > 0 && !item.classList.contains('selected')) {
                        // 多项选中中，点击新项作为多选（模拟 Ctrl）
                        this.toggleItemSelection(item);
                    } else if (item.classList.contains('selected')) {
                        // 已选中的项点击再次打开
                        const id = item.dataset.id;
                        const node = State.findNode(id);
                        if (node && node.type === 'folder') this.navigateTo(node);
                        else if (node && node.type === 'file') this.openFile(node);
                    } else {
                        this.deselectAllItems();
        ...
        if (this.isTouchDevice) {
            ...
            filesList.addEventListener('touchstart', (e) => {
                const item = e.target.closest('.file-item');
                if (!item) return;
                this._longPressTriggered = false;
            ...
            filesList.addEventListener('touchmove', (e) => {
                const touch = e.touches[0];
            ...
            filesList.addEventListener('touchend', () => {
                clearTimeout(this._longPressTimer);
            });
            
            filesList.addEventListener('touchcancel', () => {
                clearTimeout(this._longPressTimer);
            });
        }
        ...
        filesContent.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.files-touch-action-bar')) return;
            if (this.isTouchDevice && this.selectedItems.length > 0) {
                this.deselectAllItems();
                this.updateTouchActionBar();
            }
        });
        ...
        // 触屏底部操作栏事件
        const touchBar = document.getElementById(`${this.windowId}-files-touch-bar`);
        if (touchBar) {
            touchBar.addEventListener('click', (e) => {
                ...
                if (action === 'delete') {
                    // 删除需要确认，通过 handleContextAction 已处理
                }
                this.handleTouchBarAction(action);
            });
        }
    ...
    updateTouchActionBar() {
        const bar = document.getElementById(`${this.windowId}-files-touch-bar`);
        if (!bar) return;
        ...
        if (selectedCount === 0) {
            ...
            if (content) content.classList.remove('touch-bar-open');
            return;
        }

        // 确保 contextTargetId 同步为第一个选中的
        if (this.contextTargetId === null && this.selectedItems[0]) {
            this.contextTargetId = this.selectedItems[0].dataset.id;
        }

        const isRecycle = this.currentNode && this.currentNode.id === 'recycle';
        const isMulti = selectedCount > 1;
        const ids = this.selectedItems.map(el => el.dataset.id);
        const hasSystemFolder = this.currentNode && this.currentNode.id === 'root' &&
                                ids.some(id => this.SYSTEM_ROOT_FOLDERS.includes(id));

        // 根据当前环境和选中状态，动态显示按钮
        let buttonsHTML = '';
        const disabledBase = (isMulti || isRecycle) ? 'disabled' : '';

        if (isRecycle) {
            buttonsHTML = `
                <button class="touch-bar-btn ${disabledBase}" data-action="restore" title="${t('files.restore')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Reload.svg" alt="">
                    <span>${t('files.restore')}</span>
                </button>
                <button class="touch-bar-btn danger" data-action="delete-permanent" title="${t('files.delete-permanent')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Trash.svg" alt="">
                    <span>${t('files.delete-permanent')}</span>
                </button>
                <button class="touch-bar-btn ${disabledBase}" data-action="properties" title="${t('files.properties')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Information Circle.svg" alt="">
                    <span>${t('files.properties')}</span>
                </button>
                <button class="touch-bar-btn" data-action="${selectedCount > 0 ? 'deselect-all' : 'select-all'}" title="${selectedCount > 0 ? t('files.deselect-all') : t('files.select-all')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Select Box.svg" alt="">
                    <span>${selectedCount > 0 ? t('files.deselect-all') : t('files.select-all')}</span>
                </button>
            `;
        } else {
            const deleteDisabled = hasSystemFolder ? 'disabled' : '';
            buttonsHTML = `
                <button class="touch-bar-btn ${disabledBase}" data-action="open" title="${t('files.open')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Folder Open.svg" alt="">
                    <span>${t('files.open')}</span>
                </button>
                <button class="touch-bar-btn" data-action="copy" title="${t('files.copy')}">
                ...
                <button class="touch-bar-btn ${isMulti || hasSystemFolder ? 'disabled' : ''}" data-action="rename" title="${t('files.rename')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Edit.svg" alt="">
                    <span>${t('files.rename')}</span>
                </button>
                <button class="touch-bar-btn danger ${deleteDisabled}" data-action="delete" title="${t('files.delete')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Trash.svg" alt="">
                    <span>${t('files.delete')}</span>
                </button>
                <button class="touch-bar-btn ${disabledBase}" data-action="properties" title="${t('files.properties')}">
                ...
                <button class="touch-bar-btn" data-action="deselect-all" title="${t('files.deselect-all')}">
                    <img src="Theme/Icon/Symbol_icon/stroke/Select Box.svg" alt="">
                    <span>${t('files.deselect-all')}</span>
                </button>
            `;
        }

        const inner = bar.querySelector('.touch-bar-inner');
        if (inner) inner.innerHTML = buttonsHTML;

        bar.classList.add('visible');
        if (content) content.classList.add('touch-bar-open');
    },

    // 处理触屏操作栏动作
    handleTouchBarAction(action) {
        if (this.selectedItems.length === 0 && !['deselect-all', 'select-all'].includes(action)) return;

        // 将当前选中的第一个作为 contextTargetId
        ...
        switch (action) {
            case 'open':
                if (this.selectedItems.length === 1) {
                    const id = this.selectedItems[0].dataset.id;
                    const node = State.findNode(id);
                    if (node) {
                        if (node.type === 'folder') this.navigateTo(node);
                        else this.openFile(node);
                    }
                    this.deselectAllItems();
                } else {
                    FluentUI.Toast({ title: t('files.title'), message: t('files.select-one'), type: 'warning' });
                }
                break;
            case 'copy':
            ...
            case 'delete':
                if (this.selectedItems.length === 1) {
                    ...
                    this.handleContextAction('delete-selected');
                }
                this.deselectAllItems();
                break;
            case 'delete-permanent':
                if (this.selectedItems.length === 1) {
                    this.handleContextAction('delete-permanent');
                } else {
            ...
            case 'restore':
                if (this.selectedItems.length === 1) {
                    this.handleContextAction('restore');
                } else {
                    this.selectedItems.forEach(el => this.restoreNode(el.dataset.id));
                }
                this.deselectAllItems();
                break;
        }
    ...
    selectItem(item) {
        ...
        if (this.isTouchDevice) this.updateTouchActionBar();
    },
    ...
    async deleteSelectedItems() {
        ...
        // protected 文件需要 root 权限
        if (protectedNodes.length > 0 && !(State.isRoot && State.isRoot())) {
            State.addNotification({
                title: t('files.title'),
                message: '需要 root 权限才能删除此系统文件',
                type: 'warning'
            });
            return;
        }
        ...
        // 对 protected 文件做快照（5秒后自动恢复）
        protectedNodes.forEach(node => {
            this._protectedSnapshots[node.id] = {
        ...
        if (protectedNodes.length > 0) {
            State.addNotification({
                title: t('files.title'),
                message: '已删除受保护系统文件，将在 5 秒后自动恢复',
                type: 'warning'
            });
            // 5秒后自动恢复 protected 文件
            setTimeout(() => {
                this._restoreProtectedFiles();
            }, 5000);
        } else {
            let notifMsg = t('files.deleted', {count});
            // 检查是否删除了普通系统文件
            const hasSystemFile = normalNodes.some(n => n.system);
            if (hasSystemFile) {
                notifMsg = '已删除系统文件。部分应用可能无法正常工作，请在 设置-恢复出厂设置 中恢复。';
            }
            State.addNotification({
                title: t('files.title'),
                message: notifMsg,
                type: hasSystemFile ? 'warning' : 'info'
            });
        }
    },

    // 自动恢复被 root 删除的 protected 系统文件
    _restoreProtectedFiles() {
        const snapshots = this._protectedSnapshots;
        this._protectedSnapshots = {};
        Object.keys(snapshots).forEach(nodeId => {
            const snapshot = snapshots[nodeId];
            // 检查文件是否还在回收站
            const recycle = State.findNode('recycle');
            if (recycle && Array.isArray(recycle.children)) {
                const idx = recycle.children.findIndex(c => c.id === nodeId);
                if (idx !== -1) {
                    // 从回收站移除
                    const node = recycle.children.splice(idx, 1)[0];
                    delete node._recycle;
                    // 放回原父目录
                    const parent = State.findNode(snapshot.parentId);

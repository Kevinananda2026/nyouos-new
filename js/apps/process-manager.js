...
const ProcessManagerApp = {
    ...
    updateEndTaskButton(rows = this.getVisibleProcesses()) {
        ...
        const canEnd = Boolean(selected);
        button.disabled = !canEnd;
        button.classList.toggle('can-end', canEnd);
        button.classList.toggle('core-warning', Boolean(selected?.core));
    ...
    endSelectedProcess() {
        ...
        if (!selected) return;
        if (selected.core) {
            if (typeof BlueScreen !== 'undefined' && BlueScreen.show) {
                BlueScreen.show(selected.name);
            }
            return;
        }
    ...
    addStyles() {
        ...
        style.textContent = `
            ...
            .process-end-task-button.core-warning:not(:disabled) { color: #fff; background: #c50f1f; border-color: #c50f1f; }
            .process-end-task-button.core-warning:not(:disabled):hover { filter: brightness(1.1); }

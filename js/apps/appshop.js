...
const AppShop = {
    ...
    getCurrentCountryCode() {
        try {
            ...
            if (s?.countryRegion) return s.countryRegion;
        } catch (_) {}
        return 'CN';
    },
    ...
    getRegionApps() {
        try {
            if (typeof Countries !== 'undefined') {
                ...
                return Countries.getAppsForCountry(code);
            }
        } catch (_) {}
        return [];
    },
    ...
    getCatalogApps() {
        const regionApps = this.getRegionApps();
        const catalogApps = regionApps.map(app => ({
            rating: app.rating || 4.5,
            downloads: app.downloads || '10万+',
            featured: app.featured !== false,
            banner: app.banner === true,
            isPWA: true,
            width: 1100,
            height: 760,
            ...app
        ...
        if (Array.isArray(catalog) && catalog.length > 0) {
            const nativeIds = new Set(catalog.filter(a => a.isNative || a.isSystem).map(a => a.id));
            const filtered = catalog.filter(a => !a.isNative && !a.isSystem);
            const merged = [...filtered, ...catalogApps];
            const seen = new Set();

...
const Surf = {
    ...
    conversations: [],
    currentId: null,
    STORAGE_KEY: 'fluentos.Surf_history',
    ...
    badWords: [
        '操', '靠', '妈的', '他妈的', '你妈的', '傻逼', '傻逼', '草泥马',
        'fuck', 'shit', 'bitch', 'asshole', 'damn', 'bullshit',
        '白痴', '笨蛋', '傻瓜', '蠢', '滚', '去死', '垃圾', '废物',
        'バカ', 'アホ', 'クソ', 'くそ', 'ちくしょう', 'ばか', 'あほ', '死ね', 'くそったれ',
        'สัตว์', 'ควย', 'หี', 'เหี้ย', 'ไอ้บ้า', 'บ้า', 'ตาย', 'สาด', 'ห่วย',
        'Scheiße', 'Arschloch', 'Wichser', 'Idiot', 'Dummkopf', 'Schwanz', 'Fick', 'Hurensohn', 'Miststück'
    ],

    init() {
        this.element = document.getElementById('Surf-panel');
        this.blurLayer = document.getElementById('Surf-blur-layer');
        this.input = document.getElementById('Surf-input');
        this.messagesEl = document.getElementById('Surf-messages');
        this.historyEl = document.getElementById('Surf-history');
        this.contentEl = this.element?.querySelector('.Surf-content');
        this._updateInputPlaceholder();
        State.on('languageChange', () => this._updateInputPlaceholder(), { key: 'Surf.languageChange' });
    ...
    show() {
        ...
        this.element.classList.remove('hidden', 'Surf-closing');
        this._ensurePanelForeground();
        if (this.blurLayer) this.blurLayer.classList.add('Surf-visible');
    ...
    hide(reason = 'external') {
        ...
        if (this.blurLayer) this.blurLayer.classList.remove('Surf-visible');
        const btn = document.getElementById('Surf-btn');
        if (btn) btn.classList.remove('active');

        if (State.settings.enableAnimation) {
            this.element.classList.add('Surf-closing');
            setTimeout(() => {
                this.element.classList.add('hidden');
                this.element.classList.remove('Surf-closing');
            }, 200);
        } else {
            this.element.classList.add('hidden');
        }
    ...
    bindEvents() {
        ...
        document.addEventListener('click', (e) => {
            ...
            if (this.isOpen && !this.element.contains(e.target) && !e.target.closest('#Surf-btn') && !e.target.closest('#Surf-context-menu')) {
        ...
        // 工具栏按钮
        document.getElementById('Surf-new-chat')?.addEventListener('click', () => this.newConversation());
        document.getElementById('Surf-history-btn')?.addEventListener('click', () => this._toggleHistory());
        document.getElementById('Surf-clear-btn')?.addEventListener('click', () => this.clearAll());
    },
    ...
    _ensureContextMenu() {
        ...
        menu.id = 'Surf-context-menu';
        menu.className = 'context-menu hidden';
        menu.addEventListener('contextmenu', (e) => e.preventDefault());
        menu.addEventListener('click', (e) => {
            const item = e.target.closest('.context-menu-item');
            if (!item) return;
            if (item.dataset.action === 'open-Surf-settings') {
                e.preventDefault();
                e.stopPropagation();
                this._hideContextMenu();
                this._executeAction('openSettings:Surf');
            }
        });
    ...
    _renderContextMenu() {
        ...
        const label = this.lang() === 'zh' ? '打开Surf AI设置' : 'Open Surf AI Settings';
        this.contextMenuEl.innerHTML = `
            <div class="context-menu-item" data-action="open-Surf-settings">
    ...
    _setEmptyState(empty) {
        if (!this.contentEl) return;
        const wasEmpty = this.contentEl.classList.contains('Surf-empty');
        if (wasEmpty === empty) return;

        clearTimeout(this._cardAnimTimer);
        this.contentEl.classList.remove('Surf-expanding', 'Surf-collapsing');
        this.contentEl.classList.toggle('Surf-empty', empty);

        const animClass = empty ? 'Surf-collapsing' : 'Surf-expanding';
    ...
    lang() {
        const lang = I18n && I18n.currentLang;
        if (['zh', 'en', 'ja', 'th', 'de'].includes(lang)) return lang;
        return 'zh';
    },
    ...
    _badWordReply() {
        const replies = {
            zh: '这样说很不好。',
            en: 'That\'s not nice to say.',
            ja: 'そんな言い方はよくないですよ。',
            th: 'พูดแบบนั้นไม่ดีนะ',
            de: 'Das ist nicht nett, so zu sprechen.'
        };
        return replies[this.lang()] || replies['zh'];
    },
    ...
    _updateInputPlaceholder() {
        ...
        this.input.placeholder = placeholders[this.lang()] || placeholders['zh'];
    },
    ...
    getApiKeyStorageType() {
        if (State.settings.SurfApiStorageType) return State.settings.SurfApiStorageType;
        if (State.settings.SurfApiEncrypted && State.settings.SurfApiEncrypted.ciphertext) return 'permanent-encrypted';
        if ((State.settings.SurfApiKey || '').trim()) return 'permanent-plain';
        return 'none';
    },
    ...
    saveApiKeyTemporary(apiKey) {
        ...
        State.updateSettings({
            ...
            SurfApiStorageType: 'session'
        });
        State.emit('SurfApiKeyReady', { storageType: 'session', decrypted: true });
        return true;
    },
    ...
    clearApiKey() {
        ...
        State.emit('SurfApiKeyReady', { storageType: 'none', decrypted: false });
    },
    ...
    async saveApiKeyPermanentEncrypted(apiKey, passphrase) {
        ...
        State.updateSettings({
            SurfApiKey: '',
            SurfApiEncrypted: encrypted,
            SurfApiStorageType: 'permanent-encrypted'
        });
        State.emit('SurfApiKeyReady', { storageType: 'permanent-encrypted', decrypted: true });
        return true;
    },
    ...
    async getApiKeyForRequest() {
        ...
        if ((State.settings.SurfApiKey || '').trim()) {
            State.updateSettings({ SurfApiKey: '', SurfApiStorageType: 'none' });
        }

        const encryptedPayload = State.settings.SurfApiEncrypted;
    ...
    _createMessageElement(text, type) {
        ...
        div.className = `Surf-msg Surf-msg-${type}`;
        ...
        if (type === 'bot') {
            div.classList.add('Surf-msg-copyable');
            const copyBtn = document.createElement('button');
            copyBtn.type = 'button';
            copyBtn.className = 'Surf-copy-btn';
            const tip = this.lang() === 'zh' ? '复制内容' : 'Copy message';
            copyBtn.title = tip;
            copyBtn.setAttribute('aria-label', tip);

            const icon = document.createElement('img');
            icon.className = 'Surf-copy-icon';
    ...
    containsBadWord(text) {
        ...
        return this.badWords.some(word => lower.includes(word.toLowerCase()));
    },

    addThinkingMessage() {
        const div = document.createElement('div');
        div.className = 'Surf-msg Surf-msg-bot Surf-msg-thinking';
        
        const marquee = document.createElement('div');
        marquee.className = 'Surf-thinking-marquee';
        ...
        span.textContent = thinkingTexts[this.lang()] || thinkingTexts['zh'];
        textEl.appendChild(span);
        
        const dots = document.createElement('span');
        dots.className = 'Surf-thinking-dots';
    ...
    _renderHistoryList() {
        ...
        if (!this.conversations.length || (this.conversations.length === 1 && !this.conversations[0].messages.length)) {
            this.historyEl.innerHTML = `<div class="Surf-history-empty">${this.lang() === 'zh' ? '暂无历史记录' : 'No history'}</div>`;
            return;
        }
        this.conversations.forEach(c => {
            if (!c.messages.length) return;
            const item = document.createElement('div');
            item.className = 'Surf-history-item';
    ...
    _normalizeInputText(text) {
        ...
        normalized = normalized
            ...
            .replace(/\bSurf\s*ai\b/g, 'Surf ai')
            .replace(/Surfai/g, 'Surf ai')
            .replace(/全屏幕/g, '全屏')
    ...
    buildPreviewReply(rawText, options = {}) {
        ...
        const fallbackText = options.fallbackText
            || resolvePayload(typeof SurfData !== 'undefined' ? SurfData.fallback : null)
    ...
    async processInput(text) {
        ...
        if (this.containsBadWord(text)) {
            setTimeout(() => {
                this.addMessage(this._badWordReply(), 'bot');
            }, 300);
            return;
        }
        ...
        if (this._pendingAction) {
            if (this._pendingAction.type === 'offerQuickStart') {
                const lower = text.toLowerCase();
                const isYes = SurfData.confirmYes.some(w => lower.includes(w));
                const isNo = SurfData.confirmNo.some(w => lower.includes(w));
                if (isYes || isNo) {
        ...
        if (State.settings.SurfCustomMode) {
        ...
        const fallbackText = this._resolveResponse(SurfData.fallback) || (this.lang() === 'zh' ? '抱歉，我暂时没听懂。' : 'Sorry, I did not understand.');
        setTimeout(() => this.addMessage(fallbackText, 'bot'), 400);
    },

    // --- 快捷键汇总/开始菜单快捷键 ---
    '_handle_shortcutsHelp'() {
        const cmd = SurfData.commands.shortcutsHelp;
        if (!cmd) return;
        const botReply = this._resolveResponse(cmd.response);
        if (botReply) {
            setTimeout(() => this.addMessage(botReply, 'bot'), 400);
    ...
    '_handle_openApp'(_text, lower) {
        const lang = this.lang();
        const cmd = SurfData.commands.openApp;
    ...
    '_handle_uninstall'(text, lower) {
        ...
        if (SurfData.systemApps.includes(app.id)) {
            setTimeout(() => this.addMessage(SurfData.commands.uninstall.responseFail[lang].replace('{app}', appName), 'bot'), 400);
            return;
        }
    ...
    _doUninstall(app, appName) {
        ...
        if (typeof StartMenu !== 'undefined') StartMenu.renderApps();
        setTimeout(() => this.addMessage(SurfData.commands.uninstall.response[lang].replace('{app}', appName), 'bot'), 400);
    },
    ...
    _doRepair(app, appName) {
        ...
        setTimeout(() => this.addMessage(SurfData.commands.repair.response[lang].replace('{app}', appName), 'bot'), 400);
    },

    // --- 壁纸 ---
    async '_handle_wallpaper'() {
        const lang = this.lang();
        setTimeout(() => this.addMessage(SurfData.commands.wallpaper.response[lang], 'bot'), 300);
        try {
    ...
    _syncControlCenterSliders() {
        ...
        if (typeof ControlCenter !== 'undefined' && typeof ControlCenter.updateTiles === 'function') {
            try {
                ...
                // Control center UI may not be mounted while Surf is running.
            }
        }
    },
    ...
    _buildCustomApiPayload(text, options = {}) {
        const provider = options.provider || State.settings.SurfProvider || 'openai';
    ...
    async _callApi(text, apiKey) {
        ...
        const sysMsg = {
            role: 'system',
            content: 'You are Surf, a helpful assistant built into FluentOS. Reply concisely. If user asks about shortcuts, provide this mapping: Alt opens Start Menu; Alt+F Surf AI; Alt+I Settings; Alt+L lock screen; Alt+E Files; Alt+A Control Center; Ctrl+A select all files/icons in Desktop or Files; Alt+D minimize all windows; Alt+M minimize topmost window; Alt+W Task View. If user asks about fullscreen, ask whether to disable "Auto Web Fullscreen On Boot" and wait for yes/no confirmation. If user asks what languages you support, say you currently support Chinese and English and are learning more languages. On greeting, proactively offer a quick-start guide and ask yes/no.'
        };
        ...
        // 显示加载占位（跑马灯动画）
        const loadingMsg = this.addThinkingMessage();

        const _updateReply = (txt) => {
            if (loadingMsg) {
                const textEl = loadingMsg.querySelector('.Surf-msg-text');
                if (textEl) {
                    textEl.textContent = '';
                    txt.split('\n').forEach((line, i) => {
                        if (i > 0) textEl.appendChild(document.createElement('br'));

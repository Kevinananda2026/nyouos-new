...
var State = {
    ...
    // 初始化
    init() {
        // 从旧版本迁移数据（通过 sessionStorage 传递）
        try {
            const keys = Object.keys(sessionStorage);
            const migrationKey = keys.find(k => k.startsWith('nyouos_migration_'));
            if (migrationKey) {
                const data = JSON.parse(sessionStorage.getItem(migrationKey) || '{}');
                ...
                console.log('[State] Migrated data from previous version');
            }
        } catch (e) {
            console.warn('[State] Data migration failed:', e);
        }
    ...
    getResolvedWallpaper(slot) {
        ...
        return typeof WallpaperStore !== 'undefined'
            ...
            : (id === 'lock' ? 'Theme/Picture/Fluent-3.jpg' : 'Theme/Picture/Fluent-5.png');
    },
    ...
    async setWallpaper(slot, source, meta = {}) {
        ...
        if (typeof WallpaperStore === 'undefined' || WallpaperStore.isBuiltIn(source)) {
            const fallback = id === 'lock' ? 'Theme/Picture/Fluent-3.jpg' : 'Theme/Picture/Fluent-5.png';
    ...
    // 确保系统文件结构完整（用户删除后恢复出厂设置时重建）
    ensureSystemFiles() {
        const root = this.fs.root;
        let sysFolder = root.children.find(c => c.id === 'system');
        if (!sysFolder) {
            sysFolder = { id: 'system', name: '系统', type: 'folder', system: true, children: [] };
            root.children.push(sysFolder);
        }
        const now = new Date().toISOString();
        const ensureSysFolder = (parent, id, name, protectedFlag) => {
            let node = parent.children.find(c => c.id === id);
            if (!node) {
                node = { id, name, type: 'folder', children: [], ...(protectedFlag ? { protected: true } : { system: true }) };
                parent.children.push(node);
            }
            return node;
        };
        const ensureSysFile = (parent, id, name, content, size, flag) => {
            let node = parent.children.find(c => c.id === id);
            if (!node) {
                node = { id, name, type: 'file', content, size, created: now, modified: now, ...(flag ? { protected: true } : { system: true }) };
                parent.children.push(node);
            }
            return node;
        };
        // protected 系统文件夹（root 才能删除，5秒自动恢复）
        const boot = ensureSysFolder(sysFolder, 'sys-boot', 'Boot', true);
        ensureSysFile(boot, 'sys-boot-efi', 'boot.efi', '[NyouOS Boot Loader]\nVersion: 26.4\nKernel: nyx-kernel-26.4', 128, true);
        ensureSysFile(boot, 'sys-boot-kernel', 'kernel.img', 'NyouOS Kernel Image v26.4', 4096, true);
        ensureSysFile(boot, 'sys-boot-config', 'config.sys', '[boot]\ntimeout=3\ndefault=nyouos\nverbose=false', 64, true);
        const bin = ensureSysFolder(sysFolder, 'sys-bin', 'Bin', true);
        ensureSysFile(bin, 'sys-bin-shell', 'shell.exe', 'NyouOS Shell Runtime', 256, true);
        ensureSysFile(bin, 'sys-bin-terminal', 'terminal.exe', 'NyouOS Terminal Emulator', 256, true);
        ensureSysFile(bin, 'sys-bin-explorer', 'explorer.exe', 'NyouOS File Explorer', 256, true);
        const lib = ensureSysFolder(sysFolder, 'sys-lib', 'Lib', true);
        ensureSysFile(lib, 'sys-lib-system', 'system.dll', 'NyouOS System Library', 2048, true);
        ensureSysFile(lib, 'sys-lib-core', 'core.framework', 'NyouOS Core Framework v26.4', 4096, true);
        const drivers = ensureSysFolder(sysFolder, 'sys-drivers', 'Drivers', true);
        ensureSysFile(drivers, 'sys-drv-display', 'display.drv', 'Display Driver v2.0', 512, true);
        ensureSysFile(drivers, 'sys-drv-audio', 'audio.drv', 'Audio Driver v1.5', 384, true);
        ensureSysFile(drivers, 'sys-drv-network', 'network.drv', 'Network Driver v3.1', 640, true);
        const registry = ensureSysFolder(sysFolder, 'sys-registry', 'Registry', true);
        ensureSysFile(registry, 'sys-reg-system', 'system.reg', '[HKEY_LOCAL_MACHINE\\System]\nVersion=26.4\nBuild=20260728', 128, true);
        ensureSysFile(registry, 'sys-reg-user', 'user.reg', '[HKEY_CURRENT_USER]\nTheme=auto\nAccent=#0078d4', 96, true);
        // 普通系统文件夹（用户可删除，需恢复出厂设置恢复）
        const apps = ensureSysFolder(sysFolder, 'sys-apps', 'Apps', false);
        ensureSysFile(apps, 'sys-app-calc', 'calculator.app', 'NyouOS Calculator App', 512, false);
        ensureSysFile(apps, 'sys-app-notes', 'notes.app', 'NyouOS Notes App', 512, false);
        ensureSysFile(apps, 'sys-app-browser', 'browser.app', 'NyouOS Browser App', 1024, false);
        ensureSysFile(apps, 'sys-app-settings', 'settings.app', 'NyouOS Settings App', 768, false);
        ensureSysFile(apps, 'sys-app-clock', 'clock.app', 'NyouOS Clock App', 384, false);
        ensureSysFile(apps, 'sys-app-weather', 'weather.app', 'NyouOS Weather App', 512, false);
        ensureSysFile(apps, 'sys-app-files', 'files.app', 'NyouOS Files App', 768, false);
        ensureSysFile(apps, 'sys-app-photos', 'photos.app', 'NyouOS Photos App', 640, false);
        const frameworks = ensureSysFolder(sysFolder, 'sys-frameworks', 'Frameworks', false);
        ensureSysFile(frameworks, 'sys-fw-ui', 'ui.framework', 'NyouOS UI Framework', 2048, false);
        ensureSysFile(frameworks, 'sys-fw-auth', 'auth.framework', 'NyouOS Auth Framework', 1024, false);
        ensureSysFile(frameworks, 'sys-fw-media', 'media.framework', 'NyouOS Media Framework', 1536, false);
        const prefs = ensureSysFolder(sysFolder, 'sys-prefs', 'Preferences', false);
        ensureSysFile(prefs, 'sys-pref-system', 'system.pref', '<?xml version="1.0"?>\n<preferences>\n  <theme>auto</theme>\n  <accent>#0078d4</accent>\n</preferences>', 128, false);
        ensureSysFile(prefs, 'sys-pref-user', 'user.pref', '<?xml version="1.0"?>\n<preferences>\n  <language>zh-CN</language>\n</preferences>', 96, false);
        const logs = ensureSysFolder(sysFolder, 'sys-logs', 'Logs', false);
        ensureSysFile(logs, 'sys-log-system', 'system.log', '[2026-07-28 00:00:00] System boot complete.\n[2026-07-28 00:00:01] All services started.', 128, false);
        ensureSysFile(logs, 'sys-log-error', 'error.log', '[2026-07-28 00:00:00] No errors.', 48, false);
    },
    ...
    ensureSettingsDefaults() {
        ...
        if (this.settings.SurfCustomMode === true && this.settings.SurfCustomLastEnabled !== true) {
            this.settings.SurfCustomLastEnabled = true;
            changed = true;
        }

        if (!this.settings.SurfApiStorageType) {
            if (this.settings.SurfApiEncrypted && this.settings.SurfApiEncrypted.ciphertext) {
                this.settings.SurfApiStorageType = 'permanent-encrypted';
            } else {
                this.settings.SurfApiStorageType = 'none';
            }
            changed = true;
        }

        if ((this.settings.SurfApiKey || '').trim()) {
            // Security hardening: plain-text permanent API key storage is no longer supported.
            this.settings.SurfApiKey = '';
            if (this.settings.SurfApiStorageType === 'permanent-plain') {
                this.settings.SurfApiStorageType = 'none';
            }
            changed = true;
        }
    ...
    restoreStrictCspOnStartup() {
        ...
        if (
            this.settings.strictCspEnabled === true
            && this.settings.SurfCustomMode !== true
            && this.settings.SurfCustomLastEnabled === true
        ) {
            this.settings.SurfCustomMode = true;
            changed = true;
    ...
    updateSettings(updates) {
        ...
        const turningOffCustomMode = safeUpdates.SurfCustomMode === false
            && this.settings
            && this.settings.SurfCustomMode === true;
        ...
        if (safeUpdates.SurfCustomMode === true) {
            if (!Object.prototype.hasOwnProperty.call(safeUpdates, 'SurfCustomLastEnabled')) {
                safeUpdates.SurfCustomLastEnabled = true;
            }
        } else if (safeUpdates.SurfCustomMode === false) {
            if (!Object.prototype.hasOwnProperty.call(safeUpdates, 'SurfCustomLastEnabled')) {
                safeUpdates.SurfCustomLastEnabled = false;
            }
        }
        if (turningOffCustomMode) {
            // Security hardening: when custom mode is disabled, clear all API key material.
            safeUpdates.SurfApiKey = '';
            safeUpdates.SurfApiEncrypted = null;
            safeUpdates.SurfApiStorageType = 'none';
        }
        ...
        if (turningOffCustomMode) {
            if (typeof window !== 'undefined' && window.Surf) {
                window.Surf._sessionApiKey = '';
                window.Surf._pendingDecryptPromise = null;
            }
            this.emit('SurfApiKeyReady', { storageType: 'none', decrypted: false });
        }
    ...
    // 检查当前用户是否拥有 root 权限
    isRoot() {
        return this.session && this.session.isRoot === true;
    },

    // 根据 Supabase display_name 判断并设置 root 权限
    checkRootPermission(displayName) {
        const isRoot = String(displayName || '').trim() === 'KevinAnanda';
        this.updateSession({ isRoot });
        return isRoot;
    },
